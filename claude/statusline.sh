...
if git -C "$current_dir" rev-parse --git-dir > /dev/null 2>&1; then
  ...
  case "$git_dir" in /*) abs_git_dir="$git_dir" ;; *) abs_git_dir="$current_dir/$git_dir" ;; esac
  case "$common_dir" in /*) abs_common_dir="$common_dir" ;; *) abs_common_dir="$current_dir/$common_dir" ;; esac
...
if [ -n "$effort" ]; then
  printf "🤖 %s | 💪 %s | 🧠 %s | ⏱️ %s\n📁 %s | %s %s" "$model" "$effort" "$usage_str" "$rate_limit_str" "$dir_display" "$branch_icon" "$git_str"
else
  printf "🤖 %s | 🧠 %s | ⏱️ %s\n📁 %s | %s %s" "$model" "$usage_str" "$rate_limit_str" "$dir_display" "$branch_icon" "$git_str"
fi
