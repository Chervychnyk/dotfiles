...
When the main agent delegates to subagents, the subagent `toolResult` contains rich metadata in a `details` field.
...
Each result object may contain:
...
| `messages` | array | Inline conversation |
| `model` | string | Model used |
| `usage` | object | Token and cost data |
...
- The first line is metadata, not a message.
- Assistant content can mix text, thinking, and tool calls.
- Some subagent session paths may point to cleaned-up temp files; prefer persistent artifact paths when available.
- Searching only assistant text can miss important user constraints.
