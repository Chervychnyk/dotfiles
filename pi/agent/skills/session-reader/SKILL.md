---
name: session-reader
description: Efficiently read and analyze pi agent session JSONL files. Use when asked to "read a session", "review a session", "analyze a session", "what happened in this session", "load session", "parse session", "session history", "go through sessions", "find a session", or given a .jsonl session file path.
---
...
Use the helper script to locate likely sessions by project path, filename, or text:

```bash
uv run ${CLAUDE_SKILL_ROOT}/scripts/find_sessions.py --project dotfiles
uv run ${CLAUDE_SKILL_ROOT}/scripts/find_sessions.py --text handoff
uv run ${CLAUDE_SKILL_ROOT}/scripts/find_sessions.py --since 7d --limit 20
```

If you already know the exact file, skip to step 2.
...
# Search for specific topic inside one session
...
1. `find_sessions.py` → find candidate sessions
2. `--mode toc` → scan the session, find interesting exchanges
3. Default (conversation) → read the human-readable flow
4. `--mode turn --turn N` → drill into specific exchanges
5. `--mode subagents` → review delegated work and follow subagent session paths
