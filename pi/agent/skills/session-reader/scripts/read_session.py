...
def print_conversation(exchanges: list[dict], args):
    ...
    for ex in filtered:
        ...
        if assistant_texts:
            hint = f"  [used: {', '.join(sorted(tool_names))}]" if has_tools else ""
            print(f"\n🤖 ASSISTANT{hint}")
...
def print_turn_detail(exchanges: list[dict], args):
    turn_num = args.turn
...
def print_subagents(messages: list[dict]):
    print(f"{'═' * 70}")
    print("SUBAGENT RUNS")
    print(f"{'═' * 70}")
...
def find_issues(exchanges: list[dict]) -> list[dict]:
    ...
    for ex in exchanges:
        ...
        user_flags = ["doesn't work", "didn't work", "not working", "broken", "failed", "error", "bug", "wrong", "issue", "problem", "crash", "fix", "still ", "again"]
...
def print_issues(exchanges: list[dict]):
    ...
    print(f"\n{'═' * 70}")
    print("Drill into any exchange: --mode turn --turn N")
...
def main():
    ...
    if args.mode == "conversation":
        ...
        if not args.turn:
            ...
            sys.exit(1)
        print_turn_detail(exchanges, args)
    elif args.mode == "issues":
        print_issues(exchanges)
