...
function inferDefaultService(services: string[], preferredService?: string): string | undefined {
  if (preferredService && services.includes(preferredService)) return preferredService
...
async function detectComposeCommand(
  ...
  const dockerCompose = await execInDir(pi, composeDir, ['docker', 'compose', 'version']).catch(() => null)
  if (dockerCompose && dockerCompose.code === 0) return 'docker compose'

  const legacy = await execInDir(pi, composeDir, ['docker-compose', 'version']).catch(() => null)
...
async function listServices(
  ...
  if (result.code !== 0) {
    throw new Error(result.stderr?.trim() || result.stdout?.trim() || 'Failed to list Docker services')
  }
...
async function listRunningServices(
  ...
  if (composeCommand === 'docker compose') return { services: uniq(services), known: true }

  return {
    services: uniq(
      services.filter(
        (line) => !/^name\s+/i.test(line) && !/^(----|\s*$)/.test(line) && !/\bExit\b|\bCreated\b/i.test(line),
      ),
    ),
    known: true,
  }
...
export default function dockerContextExtension(pi: ExtensionAPI) {
  ...
  async function refresh(ctx: ExtensionContext): Promise<DockerContextState> {
    ...
    try {
      ...
      state = {
        ...
        defaultService: inferDefaultService(services, persisted.preferredService),
        detected: true,
      }
      return state
    } catch (error: any) {
  ...
  function requireDetectedState(): DockerContextState {
    if (!state.detected || !state.composeDir || !state.composeCommand) {
      throw new Error(state.reason || 'Docker context is not available in this project')
    }
    return state
  }
  ...
  function getComposeExecSpec(
    ...
    if (!targetService) {
      throw new Error('No Docker service selected. Use /docker-context set <service>.')
    }
    ...
    const args = mode === 'exec'
      ? current.composeCommand === 'docker compose'
        ? ['docker', 'compose', 'exec', '-T', targetService, 'sh', '-lc', shellCommand]
        : ['docker-compose', 'exec', '-T', targetService, 'sh', '-lc', shellCommand]
      : current.composeCommand === 'docker compose'
        ? ['docker', 'compose', 'run', '--rm', '-T', targetService, 'sh', '-lc', shellCommand]
        : ['docker-compose', 'run', '--rm', '-T', targetService, 'sh', '-lc', shellCommand]
  ...
  pi.registerCommand('docker-context', {
    description: 'Show or manage Docker Compose context. Usage: /docker-context [refresh|set <service>]',
    handler: async (args, ctx: ExtensionCommandContext) => {
      ...
      if (trimmed === 'refresh') {
        await refresh(ctx)
        ctx.ui.notify(formatContext(state), state.detected ? 'success' : 'warning')
        return
      }
      ...
      if (trimmed.startsWith('set ')) {
        ...
        if (!state.services.includes(service)) {
          ctx.ui.notify(`Unknown service: ${service}. Known: ${state.services.join(', ')}`, 'warning')
          return
        }
    ...
    getArgumentCompletions: (prefix: string) => {
      const options = ['refresh', ...state.services.map((service) => `set ${service}`)]
  ...
  pi.registerTool({
    ...
    description: 'Inspect Docker Compose services and default execution service for the current project.',
    ...
    parameters: Type.Object({
      refresh: Type.Optional(Type.Boolean({ description: 'Refresh service detection before returning context' })),
    }),
  ...
  pi.registerTool({
    name: 'docker_exec',
    label: 'Docker Exec',
    description: 'Run a shell command inside a Docker Compose service using `docker compose exec -T`.',
    promptSnippet: 'Run Rails, Python, or Node commands inside the correct Docker Compose service',
    promptGuidelines: [
    ...
    parameters: Type.Object({
      ...
      service: Type.Optional(Type.String({ description: 'Docker Compose service name. Defaults to detected default service.' })),
      workdir: Type.Optional(Type.String({ description: 'Working directory inside the container before running the command' })),
    }),
    async execute(_toolCallId, params, signal, onUpdate, ctx) {
      await refresh(ctx)
      const execSpec = getComposeExecSpec(params.command, params.service, params.workdir)
      ...
      const result = await streamCommandInDir(execSpec.composeDir, execSpec.args, signal, onUpdate)
      ...
      return {
        content: [{ type: 'text', text: summary || `service: ${targetService}` }],
        details: {
          ...state,
          service: targetService,
          command: params.command,
    ...
    renderCall(args, theme) {
      const service = (args.service as string | undefined) || state.defaultService || '?'
    ...
    renderResult(result, { expanded, isPartial }, theme) {
      ...
      const service = (details.service as string | undefined) || state.defaultService || '?'
      ...
      const header = isPartial
        ? theme.fg('warning', `running ${mode} [${service}] ${command}`)
        : exitCode === 0
          ? theme.fg('success', `done ${mode} [${service}] exit 0`)
          : theme.fg('error', `failed ${mode} [${service}] exit ${exitCode ?? '?'}`)
  ...
  pi.registerTool({
    name: 'docker_logs',
    label: 'Docker Logs',
    description: 'Stream logs from a Docker Compose service, following live output by default.',
    ...
    parameters: Type.Object({
      service: Type.Optional(Type.String({ description: 'Docker Compose service name. Defaults to detected default service.' })),
      lines: Type.Optional(Type.Number({ description: 'How many recent log lines to show before following. Default 100.' })),
      follow: Type.Optional(Type.Boolean({ description: 'Whether to follow logs live. Default true.' })),
      since: Type.Optional(Type.String({ description: 'Optional docker logs --since value, e.g. `10m`, `1h`, or RFC3339 timestamp.' })),
    }),
    async execute(_toolCallId, params, signal, onUpdate, ctx) {
      await refresh(ctx)
      const current = requireDetectedState()
      ...
      const lines = Number.isFinite(params.lines) ? Math.max(1, Math.floor(params.lines as number)) : 100
      const follow = params.follow ?? true
      const args =
        current.composeCommand === 'docker compose'
          ? ['docker', 'compose', 'logs', '--tail', String(lines), ...(params.since ? ['--since', params.since] : []), ...(follow ? ['-f'] : []), service]
          : ['docker-compose', 'logs', '--tail', String(lines), ...(params.since ? ['--since', params.since] : []), ...(follow ? ['-f'] : []), service]
      ...
      const result = await streamCommandInDir(current.composeDir!, args, signal, onUpdate)
      const summary = [
        `service: ${service}`,
        `lines: ${lines}`,
        `follow: ${follow}`,
    ...
    renderResult(result, { expanded, isPartial }, theme) {
      ...
      const header = isPartial
        ? theme.fg('warning', `streaming logs [${service}]${follow ? ' follow' : ''}`)
        : exitCode === 0
          ? theme.fg('success', `logs complete [${service}] exit 0`)
          : theme.fg('error', `logs failed [${service}] exit ${exitCode ?? '?'}`)
  ...
  pi.on('tool_call', async (event, ctx) => {
    ...
    const alreadyDockerAware = /\bdocker(\s+compose)?\b|\bdocker-compose\b/.test(command)
