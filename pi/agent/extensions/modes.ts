...
const BOOTSTRAP_MODES: Array<{
  name: ModeName
  spec: Required<Pick<ModeSpec, 'provider' | 'modelId' | 'thinkingLevel'>>
}> = [
  {
    name: 'rush',
    spec: {
...
async function loadModesFile(filePath: string): Promise<LoadedModes> {
  try {
    ...
    const currentMode =
      typeof parsed.currentMode === 'string' ? parsed.currentMode : ''
...
async function ensureRuntime(
  ...
  if (fileChanged) {
    ...
    runtime.overlayEnabled =
      !loaded.explicitlyEmptyModes &&
      orderedModeNames(loaded.data.modes).length > 0
    ...
    if (!runtime.overlayEnabled) {
      ...
      if (
        !runtime.currentMode ||
        !(runtime.currentMode in runtime.data.modes) ||
        runtime.currentMode === CUSTOM_MODE_NAME
      ) {
...
async function syncModeFromCurrentSelection(
  pi: ExtensionAPI,
  ctx: ExtensionContext,
): Promise<void> {
  await ensureRuntime(pi, ctx)
  ...
  const inferred = inferModeFromSelection(
    getCurrentSelectionSnapshot(pi, ctx),
    runtime.data,
  )
  if (inferred) {
...
async function applyMode(
  ...
  if (!runtime.overlayEnabled) {
    if (ctx.hasUI) {
      ctx.ui.notify(
        'Mode overlay is disabled (modes.json has "modes": {}). Use /mode to configure.',
        'info',
      )
    }
    return
...
async function pickColorForModeUI(
  ctx: ExtensionContext,
...
async function selectModeUI(
  ...
  while (true) {
    ...
    if (!runtime.overlayEnabled) {
      const choice = await ctx.ui.select('Mode overlay disabled', [
        MODE_UI_CONFIGURE,
        MODE_UI_BACK,
      ])
      if (!choice || choice === MODE_UI_BACK) return
    ...
    handler: async (args, ctx) => {
      ...
      if (tokens[0] === 'store') {
        ...
        if (!runtime.overlayEnabled) {
          if (ctx.hasUI)
            ctx.ui.notify(
              'Mode overlay is disabled; add a mode first in /mode configure',
              'warning',
            )
          return
        }
        ...
        if (target === CUSTOM_MODE_NAME) {
          if (ctx.hasUI)
            ctx.ui.notify(
              `Cannot store into \"${CUSTOM_MODE_NAME}\"`,
              'warning',
            )
          return
        }
