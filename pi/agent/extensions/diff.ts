...
export default function (pi: ExtensionAPI) {
  ...
  pi.registerCommand(commandName, {
    ...
    handler: async (args, ctx) => {
      ...
      const currentGitChanges = await getGitChangedFiles(pi, ctx.cwd)
      const files = [
        ...(changedFiles.size > 0 ? changedFiles : currentGitChanges),
      ].sort((a, b) =>
        toRelative(ctx.cwd, a).localeCompare(toRelative(ctx.cwd, b)),
      )
      if (files.length === 0) {
        ctx.ui.notify('No changed files in git status', 'info')
        return
      }
